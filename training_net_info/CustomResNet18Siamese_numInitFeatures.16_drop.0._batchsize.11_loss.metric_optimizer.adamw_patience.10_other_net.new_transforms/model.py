from network import ShallowNet, CustomResNet18Siamese, CustomResNet3D10, PlainResNet3D50, PlainResNet3D101, PlainDenseNet3D121, PlainDenseNet3D161, PlainResNet3D10, PlainDenseNet3DCustom
...
class Model:
    ...
    def fit(self, epochs, train_loader, val_loader, patience, run_path=None):
        ...
        on_plateau_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min', patience=4, factor=0.5)
        # decreasing_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(self.optimizer, self.lr_decay)
        cyclic_lr_scheduler = torch.optim.lr_scheduler.CyclicLR(self.optimizer, base_lr=6e-4, max_lr=1e-2, step_size_up=len(train_loader), cycle_momentum=False, gamma=self.lr_decay)
