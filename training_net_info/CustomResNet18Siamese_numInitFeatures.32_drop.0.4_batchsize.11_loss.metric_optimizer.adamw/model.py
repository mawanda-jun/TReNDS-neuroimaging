...
class Model:
    ...
    def __build_model(self,
        ...
        #     network: nn.Module = CustomDenseNet3D(dropout_prob)
            ...
            network = PlainResNet18Siamese(dropout_prob, num_init_features, use_apex)
        ...
        if optimizer_type== 'adam':
            ...
            optimizer_fn = torch.optim.Adam(params=network.parameters(), lr=lr, weight_decay=1e-2)
        elif optimizer_type== 'adamw':
            # Define the optimizer. It wants to know which parameters are being optimized.
            optimizer_fn = torch.optim.AdamW(params=network.parameters(), lr=lr, weight_decay=1e-2)
        elif optimizer_type== 'SGD':
            optimizer_fn = torch.optim.SGD(params=network.parameters(), lr=lr, momentum=0.9, weight_decay=1e-7,
                                           nesterov=True)
        else:
    ...
    def fit(self, epochs, train_loader, val_loader, patience, run_path=None):
        ...
        cyclic_lr_scheduler = torch.optim.lr_scheduler.CyclicLR(self.optimizer, base_lr=1e-3, max_lr=3e-2,
